...
def get_form_color(form_str):
    """Get color based on recent form"""
...
def get_form_streak(form_str):
    """Get current streak from form"""
    if not form_str or form_str == "-----":
        return ""
    
    streak_char = form_str[0]
    if streak_char == "-":
        return ""
    ...
    if count >= 3:
        return f"🔥{count}{streak_char}" if streak_char == "W" else f"❄️{count}{streak_char}"
    return ""
...
hdr1.caption(f"{auto_status} | {now.strftime('%I:%M:%S %p ET')} | v15.50")
...
st.caption("Last 5 games (most recent first) • W=Win L=Loss")
    ...
    for game_key in game_list:
        ...
        for team in parts:
            if team not in teams_shown:
                ...
                form = fetch_team_form(team)
                wins = form.count("W")
                form_data.append({"team": team, "form": form, "wins": wins})
    
    form_data.sort(key=lambda x: x["wins"], reverse=True)
    
    cols = st.columns(4)
    for idx, fd in enumerate(form_data):
        with cols[idx % 4]:
            form_color = get_form_color(fd["form"])
            streak = get_form_streak(fd["form"])
            st.markdown(f"""<div style="background:linear-gradient(135deg,#0f172a,#020617);padding:8px 12px;margin-bottom:4px;border-radius:6px;border-left:3px solid {form_color}">
            <b style="color:#fff">{fd['team']}</b><br>
            <span style="color:{form_color};font-weight:bold;font-family:monospace;font-size:1.2em">{fd['form']}</span>
            <span style="color:#888">({fd['wins']}-{5-fd['wins']})</span>
...
st.divider()
st.caption("⚠️ Entertainment only. Not financial advice. v15.50")
