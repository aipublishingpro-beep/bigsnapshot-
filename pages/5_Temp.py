    ...
    if settlement_low is not None and is_locked:
        ...
        if anomaly_active and forecast_temp is not None:
            ...
            if temp_gap > 3 or anomaly_words:
                bracket_info = f"Winning bracket: {bracket_name} @ {ask}¢" if winning else "No matching bracket on Kalshi"
                st.markdown(f"""
                <div style="background:linear-gradient(135deg,#2d0a0a,#1a0505);border:4px solid #ef4444;border-radius:20px;padding:40px;margin:20px 0;text-align:center;box-shadow:0 0 30px rgba(239,68,68,0.4)">
                    ...
                    <div style="color:#6b7280;font-size:0.9em;margin-top:5px">{bracket_info}</div>
...
st.markdown("---")
st.markdown('<div style="background:linear-gradient(90deg,#d97706,#f59e0b);padding:10px 15px;border-radius:8px;margin-bottom:20px;text-align:center"><b style="color:#000">🧪 FREE TOOL</b> <span style="color:#000">— LOW Temperature Edge Finder v9.3 (5 Cities)</span></div>', unsafe_allow_html=True)
