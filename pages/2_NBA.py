...
def fetch_game_boxscore(game_id):
    ...
    try:
        ...
        st.warning(f"Boxscore fetch failed: {e}")
        return {}
...
def calculate_net_rating(game):
    ...
    h_ortg = (h_pts / h_poss) * 100 if h_poss > 0 else 0
    a_ortg = (a_pts / a_poss) * 100 if a_poss > 0 else 0
    net = h_ortg - a_ortg
...
for g in games:
    ...
    if g["status"] in ["STATUS_FINAL", "STATUS_FULL_TIME"]:
        ...
        if abs(lead) >= 25:
            confidence, stars = "FORTRESS", "⭐⭐⭐"
        elif abs(lead) >= 20:
            confidence, stars = "STRONG", "⭐⭐"
        else:
            confidence, stars = "GOOD", "⭐"
...
st.caption("Early momentum at 12+ min • Pace lock + Net Rating • Catch value before prices jump")
...
for g in live_games:
    ...
    h_ortg, a_ortg, net_rating, poss = calculate_net_rating(g)
    if poss < 22:  # Stricter min for early signal (avoids noise)
        continue
    ...
    if net_rating >= 16 and poss >= 22:
        ...
        totals_side = ""
        if pace_dev >= 0.3:  # Fast pace
            totals_side = " + OVER"
        elif pace_dev <= -0.3:  # Slow pace
            totals_side = " + UNDER"
...
if secret_sauce:
    for s in sorted(secret_sauce, key=lambda x: -x['net']):  # Strongest Net first
        st.markdown(f"""
        <div style="background:#0f172a;padding:16px;border-radius:12px;border:3px solid {s['conf_color']};margin-bottom:16px">
            <div style="font-size:22px;font-weight:bold;color:#ffd700">{s['game']} • 12+ min played</div>
            ...
            <div style="margin-top:12px;color:{s['conf_color']};font-weight:bold">{s['confidence']} SAUCE</div>
    ...
    st.info("⏳ Waiting for games to hit 12+ min • Secret Sauce alerts appear automatically")
